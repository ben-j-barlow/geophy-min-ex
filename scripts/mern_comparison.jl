...
# set up the solver
coords = [CartesianIndex(5,1), CartesianIndex(15,10), CartesianIndex(35,8), CartesianIndex(35,8), CartesianIndex(45,10), CartesianIndex(40,5)]
...
savefig(p, string(save_dir, "my_ore_map.png"))

println(r_massive)

path = string(save_dir, "my_belief_mean.txt")
open(path, "w") do io
    println(io, "")
end
...
for i in 1:5
    ...
    if i % 1 == 0
        ...
        savefig(p, string(save_dir, "my_$(i)volume.png"))
        #display(p)
        empty!(p)
        
        p_mean, p_std = plot(bp)
        savefig(p_mean, string(save_dir, "my_$(i)b.png"))
        #display(p)
        empty!(p_mean)
