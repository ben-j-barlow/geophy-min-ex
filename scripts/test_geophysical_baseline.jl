...
N_PARTICLES = 500
MOVE_MULT = 5  # assuming 50ms plane, 25m grid cell lengths
...
for (i, seed) in enumerate(SEEDS[1:10])
    ...
    write_baseline_result_to_file(m, final_belief, final_state; n_fly=n_fly, reward=v, seed=seed, r_massive=r_massive, grid=GRID_LINES, which_map=:base)
